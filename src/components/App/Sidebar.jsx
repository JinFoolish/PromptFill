...
export const Sidebar = ({
  activeTab, // 'home' | 'details' | 'settings' | 'history' | 'banks'
  onHome,
  onDetail,
  onHistory,
  onBanks,
  ...
  // 如果没有提供回调函数，使用路由导航
  const handleHome = onHome || navigation.navigateToHome;
  const handleDetail = onDetail || (() => {
    // 如果有激活的模板，导航到模板页面
    const templateId = navigation.getTemplateIdFromRoute();
    if (templateId) {
      navigation.navigateToTemplate(templateId);
    } else {
      // 否则导航到首页
      navigation.navigateToHome();
    }
  });
  const handleHistory = onHistory || navigation.navigateToHistory;
  const handleBanks = onBanks || navigation.navigateToBanks;
  const handleSettings = onSettings || navigation.navigateToSettings;
  
  // 如果没有提供 activeTab，从路由中获取
  const currentTab = activeTab || navigation.getActiveTab();
  // 定义三种状态的颜色
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          >
            <div 
              ...
              className={`group-hover:bg-[#F97316] ${currentTab === 'home' ? (isDarkMode ? 'bg-[#FB923C]' : 'bg-[#EA580C]') : (isDarkMode ? 'bg-[#8E9196]' : 'bg-[#6B7280]')}`}
            />
          </button>
          ...
          >
            <div 
              ...
              className={`group-hover:bg-[#F97316] ${currentTab === 'details' ? (isDarkMode ? 'bg-[#FB923C]' : 'bg-[#EA580C]') : (isDarkMode ? 'bg-[#8E9196]' : 'bg-[#6B7280]')}`}
            />
          </button>
          ...
          >
            <div className={`${isDarkMode ? 'text-gray-400' : 'text-gray-400'} group-hover:text-[#F97316] transition-colors ${currentTab === 'history' ? 'text-[#EA580C]' : ''}`}>
          ...
          >
            <div className={`${isDarkMode ? 'text-gray-400' : 'text-gray-400'} group-hover:text-[#F97316] transition-colors ${currentTab === 'banks' ? 'text-[#EA580C]' : ''}`}>
      ...
      <div className="flex flex-col items-center gap-6 w-full">
        ...
        >
          <div 
            ...
            className={`group-hover:bg-[#F97316] ${currentTab === 'settings' ? (isDarkMode ? 'bg-[#FB923C]' : 'bg-[#EA580C]') : (isDarkMode ? 'bg-[#8E9196]' : 'bg-[#6B7280]')}`}
          />
        </button>
